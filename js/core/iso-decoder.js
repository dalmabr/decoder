...
export function getBCDLength(hexLen) {
    // Neste projeto, o "bcd" de tamanho vem como valor binÃ¡rio em hex.
    // Ex.: 0x10 => 16.
  ...
  for (const [de, c] of Object.entries(config)) {
    ...
    if (c.type === 'Fixo') {
      if (typeof c.length !== 'number')
        throw new Error(`DE ${de}: FIXO precisa de length numÃ©rico`);
      ...
      if (c.unit === 'digits' && c.format === 'n' && c.length % 2 !== 0)
        console.warn(`DE ${de}: nÃºmero Ã­mpar de dÃ­gitos em BCD FIXO`);
    }
...
function looksLikeIsoAt(hex, pos = 0, mtiBytes = 2) {
    const needed = (mtiBytes + 8) * 2; // MTI + bitmap primÃ¡rio
...
function isPlausibleMti(mti) {
    ...
    // fallback conservador: versÃ£o 0/1/2 e classes mais comuns.
    const v = mti[0];
    const cls = mti[1];
    const fn = mti[2];
...
export function detectNetwork(rawHex) {
    ...
    const visaOffsetCandidates = Array.from(new Set([
        configuredVisaHeader * 2,      // header informado em bytes (padrÃ£o esperado)
        configuredVisaHeader,          // header informado jÃ¡ em hex positions
        44,                            // legado comum (22 bytes)
        88,                            // variaÃ§Ã£o possÃ­vel (44 bytes)
        ...scannedOffsets              // fallback robusto: header variÃ¡vel
    ...
    // Fallback: se hÃ¡ ISO no inÃ­cio e MTI em BCD, trata como Visa.
...
export function parseLLVARNumericBCD(field) {
    ...
    // 1 byte BCD = 2 dÃ­gitos de tamanho
...
export function decodeISO8583(hex, network) {
    ...
    // Loop pelos 128 DEs possÃ­veis
    const localState = { pos: stateStart };
    for (let i = 1; i <= allBits.length; i++) {

        if (allBits[i - 1] === '1') {
            // DE1 indica presenÃ§a de bitmap secundÃ¡rio e jÃ¡ foi consumido acima
            if (i === 1) continue;
